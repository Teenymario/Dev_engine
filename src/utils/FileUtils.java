...
public class FileUtils {
    ...
    public static String loadAsString(String path) {
        try(BufferedReader reader = new BufferedReader(new InputStreamReader(Class.class.getResourceAsStream(path)))) {
            ...
            while((line = reader.readLine()) != null) {
                stringer.inner.append(line).append("\n");
            }
        } catch (IOException e) {
    ...
    public static void recursiveLoop(FileCallback callback, String path) {
        ...
        for (File file : resources) {
            if (file.isDirectory()) {
                if (!path.endsWith("/")) path = stringer.concat(path, "/");
                recursiveLoop(callback, stringer.concat(path, file.getName(), "/"));
            } else {
                callback.call(file);
            }
        }
    ...
    public static void recursiveLoopAlphabetic(FileCallback callback, String path) {
        ...
        for (File file : resources) {
            if (file.isDirectory()) {
                ...
                recursiveLoopAlphabetic(callback, stringer.concat(path, file.getName(), "/"));
            } else {
                callback.call(file);
            }
        }
